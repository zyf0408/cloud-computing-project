# 远程部署脚本 - 华为云 ECS
$serverIP = "119.3.216.113"
$password = "Cloud123456!"

# 使用 sshpass 或 expect 来自动输入密码
# 先安装 sshpass

Write-Host "Starting remote deployment to $serverIP..."
...
# 6. 安装 kubectl 和华为云 CLI
echo "[6/6] 安装 Kubernetes 工具..."
...
# 将脚本写入临时文件
$remoteScript | Out-File -FilePath "$env:TEMP\deploy.sh" -Encoding UTF8

Write-Host "Remote script prepared. Starting deployment..."
