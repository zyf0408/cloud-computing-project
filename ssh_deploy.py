...
def main():
    ...
    # 将部署脚本写入临时文件
    with open("C:\\Users\\周云富\\Desktop\\云计算课设\\deploy_remote.sh", "w") as f:
        f.write(DEPLOY_SCRIPT)
    
    print("部署脚本已准备完成")
    print("\n由于需要交互式密码输入，请手动执行以下命令：")
    print(f"\n  ssh {USER}@{HOST}")
    print(f"  密码: {PASSWORD}")
    print("\n登录后执行：")
    print("  bash /tmp/deploy.sh")
    
    # 尝试使用 pscp 上传脚本
    try:
        # 使用 scp 上传
        cmd = f'pscp -pw {PASSWORD} C:\\Users\\周云富\\Desktop\\云计算课设\\deploy_remote.sh {USER}@{HOST}:/tmp/deploy.sh'
